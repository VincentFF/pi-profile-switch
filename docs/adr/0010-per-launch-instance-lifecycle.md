...
运行时新建的状态用软链锚回真实 agentDir：目录（`sessions`、`missions`）在真实 agentDir 下缺失时创建；文件（`auth.json`、`models-store.json`）在 instance 内建立**允许悬空**的软链——目标是“创建该文件的路径在真实 agentDir”，Pi 看它不存在就穿透写入，文件内容与格式始终由 Pi 拥有。判定“未识别条目”时**不**比对真实 agentDir，只看条目本身是否由 pi-profile 生成。
...
**按 Pi 的空值把状态文件自己建出来**（`auth.json`、`models-store.json` 写 `{}`）。锁会落在真实路径上，看起来更干净，但把 Pi 的内部空值形状写死：Pi 以后换格式而不重写已有文件时，留下的是陈旧文件。软链方案让 Pi 自己拥有文件内容与格式。
...
- 文件类 seed 用悬空软链，锁落在软链路径旁（`<instance>/auth.json.lock`），并发 instance 不会串行化对同一个真实文件的写入；换取的是永不写死 Pi 的文件格式。
- `sessions`、`missions` 会在真实 agentDir 下被创建（原生 Pi 也会创建它们），因此该状态跨 profile 共享而非隔离。
